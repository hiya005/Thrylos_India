...
import PremiumTemplatesGallery from '@/components/PremiumTemplatesGallery';
...
const GetStarted = () => {
  ...
  // Detect premium status if already logged in (for template gating)
  useEffect(() => {
    if (!user) return;
    supabase.from('profiles').select('is_verified').eq('user_id', user.id).maybeSingle()
      .then(({ data }) => setIsPremium(!!data?.is_verified));
  }, [user]);
                  ...
                  <div className="space-y-6 animate-fade-in">
                    ...
                    <div className="pt-2 border-t border-border/40">
                      <PremiumTemplatesGallery
                        isPremium={isPremium}
                        selectedTemplate={formData.selectedService}
                        onSelect={(tpl) => setFormData({ ...formData, selectedService: tpl.name })}
                      />
                    </div>
                  </div>
                )}
