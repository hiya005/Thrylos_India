...
const isImageUrl = (icon: string) => icon && (icon.startsWith('http') || icon.startsWith('/'));
...
const Services = () => {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      setLoading(true);
      const { data } = await supabase.from('services').select('*').eq('is_active', true).order('order_index', { ascending: true });
      if (data) {
        ...
        <div className="container mx-auto px-4">
          {loading ? (
            ...
            <div className="text-center py-12">
              <p className="text-muted-foreground">We're updating our services. Please check back soon.</p>
            </div>
          ) : (
